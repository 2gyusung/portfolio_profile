...
function signup(){
  ...
  const info = document.getElementById('info__check').checked
  const infoLocation = document.getElementById('info__location').checked
  ...
  if(location !== "서울" && location !== "경기" && location !== "인천"&& location !== "대전"){
  ...
  if(info === false && infoLocation  === false) {
    document.getElementById("error__info").innerText = "개인정보 및 위치정보를 동의해주세요."
    isValid = false
  }else {
    document.getElementById("error__info").innerText = ""
  }

  if(isValid === true){
      alert(`${writer}님 가입을 축하합니다`)
      location.reload();
  }
}
...
function login() {
    ...
    const pw = document.getElementById('passwordTxt').value
    const easyLogin = document.getElementById("easyLogin").checked

    let isLogin = true

    if(id.includes("@") === false || id.length < 3) {
        document.getElementById("error__id").textContent = "아이디가 올바르지 않습니다."
        isLogin = false
    } else {
        document.getElementById("error__id").innerText = ""
    }

    if(pw.length < 3) {
        document.getElementById("error__pw").textContent = "비밀번호가 올바르지 않습니다."
        isLogin = false
    } else {
